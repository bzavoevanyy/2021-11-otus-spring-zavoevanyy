...
class AuthorControllerTest {
    ...
    @Test
    @DisplayName(" return redirect to login page for GET request /author when unauthorized")
    void shouldRedirectForGetAllAuthors() throws Exception {
        mvc.perform(get("/author"))
    ...
    @Test
    @DisplayName(" redirect to GET /login when unauthorized")
    void shouldRedirectToLoginPage() throws Exception {
        mvc.perform(post("/author")
                        .param("authorName", "test"))
                .andExpect(status().is3xxRedirection());
        verify(authorService, times(0)).createAuthor("test");
    }
    ...
    @Test
    @DisplayName(" redirect to GET /login when unauthorized")
    void shouldNotUpdateAuthorAndRedirectToLoginPage() throws Exception {
        mvc.perform(post("/author/1")
                        .param("authorName", "test"))
                .andExpect(status().is3xxRedirection());
        verify(authorService, times(0)).updateAuthor(1L, "test");
    }
    ...
    @Test
    void shouldRedirectToLoginPageWhenUnauthorizedDeleteAuthorById() throws Exception {
        mvc.perform(post("/author/delete/1")
                        .param("bookId", "1"))
                .andExpect(status().is3xxRedirection());
        verify(authorService, times(0)).deleteAuthorById(1L);
    }
